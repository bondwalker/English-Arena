      ...
      {phase === "reveal" && (
        <div style={{position:"relative",overflow:"hidden"}}>
          ...
          <div className="sa-anim-pop" style={{padding:"1.8rem 1.5rem",marginBottom:"1.1rem",borderRadius:18,background:isCorrect?"rgba(122,220,90,0.12)":"rgba(255,92,66,0.12)",border:`2.5px solid ${isCorrect?"var(--leaf)":"var(--tomato)"}`,display:"flex",flexDirection:"column",alignItems:"center",gap:"0.9rem"}}>
            ...
            {q.type==="word_match" && Array.isArray(q.pairs) && <div style={{width:"100%",display:"flex",flexDirection:"column",gap:"0.4rem"}}>{q.pairs.map((p,i)=>(<div key={i} style={{fontSize:"1rem",lineHeight:1.35,textAlign:"left"}}><strong style={{color:"var(--sun)"}}>{p.word}</strong> <span style={{color:"var(--ink-soft)"}}>— {p.meaning}</span></div>))}</div>}
